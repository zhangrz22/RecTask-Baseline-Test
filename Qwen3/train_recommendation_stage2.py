...
def setup_model_and_tokenizer(model_args, training_args=None):
    """
    设置模型和tokenizer
    
    Args:
        model_args: 模型参数
        training_args: 训练参数（用于判断是否为主进程）
        
    Returns:
        tuple: (model, tokenizer)
    """
    is_main_process = training_args is None or training_args.local_rank <= 0
